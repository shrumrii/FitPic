...
async def upload_pfp(user_id: str, image: UploadFile = File(...)):
    ...
    try:
        old_pfp_filename = ""
        pfp_check_query = await get_supabase().table("users").select("pfp_url").eq("user_id", user_id).execute()
        ...
        query = await get_supabase().table("users").update({"pfp_url": image_url}).eq("user_id", user_id).execute()
...
async def get_feed(user_id: str, include_likes: bool = True, mode: str = "recent", limit: int = 50):
    ...
    feed_list = []
    try:
        query = await get_supabase().table("follows").select("following_id").eq("follower_id", user_id).execute()
